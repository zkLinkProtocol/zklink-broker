...
chain_action() {
    ...
    tokens=`${SQL} -A -t -c "select id from (select id as id,json_array_elements_text(chains::json) as chain_id from tokens where id < 128 and id > 16) as tokens_chains where chain_id = '$chain_id';"`
    for token in $tokens
    do
	action $1 $2 $token
    done
